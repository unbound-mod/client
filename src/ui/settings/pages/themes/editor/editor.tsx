...
export default ({ manifest, bundle }) => {
    ...
    return <ReactNative.ScrollView>
        <StaticSection style={{ marginBottom: 0 }}>
            ...
            <ReactNative.Text style={[styles.header, { fontSize: 20 }]}>{i18n.Messages.UNBOUND_THEME_EDITOR_TITLE_EDITING.format({ name: manifest.name, id: manifest.id })}</ReactNative.Text>
            <ReactNative.Text style={styles.subheader}>{manifest.description}</ReactNative.Text>
            <Button 
                text={i18n.Messages.UNBOUND_THEME_EDITOR_ACTION_SAVE}
                onPress={() => {
