...
import { Files, getStore, on } from '@api/storage';
...
export function packExists<TAsync extends boolean>(store: ReturnType<typeof getStore>, pack: string, fs = false as TAsync): TAsync extends true ? Promise<boolean> : boolean {
	if (fs) {
